...
public class FaceServer {
    ...
    /**
     * 激活设备,安装后只需激活一次,卸载重装后需要重新激活
     */
    public boolean active(Context context) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                faceEngine = new FaceEngine();
                int active = faceEngine.activeOnline(context, BpContact.FACE_APP_ID, BpContact.FACE_SDK_KEY);
                if (active == ErrorInfo.MOK) {
                    faceEngine = null;
                    Log.e("Asion--active", "active: succ! code = " + active);
                    return true;
                } else {
                    faceEngine = null;
                    Log.e("Asion--active", "active: failed! code = " + active);
                }
            }
            return false;
        }
        ...
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                if (engineCode == ErrorInfo.MOK) {
                    Log.e("Asion--init", "init: succ! code = " + engineCode);
                    initFaceList(context);
                    return true;
                } else {
                    faceEngine = null;
                    Log.e("Asion--init", "init: failed! code = " + engineCode);
                    return false;
                }
            }
            return false;
        ...
        synchronized (this) {
            ...
            for (File featureFile : featureFiles) {
                try {
                    ...
                    fis.read(feature);
                    fis.close();
                    String[] split = featureFile.getName().split("-");
                    if(split.length!=0){
                        String path = ROOT_PATH + File.separator + SAVE_IMG_DIR + File.separator + featureFile.getName() + IMG_SUFFIX;
                        faceRegisterInfoList.add(new FaceRegisterInfo(feature,split[0],split[1],split[2],path));
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
    ...
    public boolean deleteFace(Context context, String num){
        synchronized (this) {
            if (context == null) {
                return false;
            }
            ...
            if (faceRegisterInfoList != null) {
                for (FaceRegisterInfo info : faceRegisterInfoList) {
                    if(info.getNum().equals(num)){
                        faceRegisterInfoList.remove(info);
                        isDelete = true;
                        break;
                    }
                }
            ...
            if (featureFileDir.exists() && featureFileDir.isDirectory()) {
                ...
                if (featureFiles != null && featureFiles.length > 0) {
                    for (File featureFile : featureFiles) {
                        if (featureFile.getAbsolutePath().contains(num)) {
                            isDelete = featureFile.delete();
                            break;
                        }
                    }
                }
            ...
            if (imgFileDir.exists() && imgFileDir.isDirectory()) {
                ...
                if (imgFiles != null && imgFiles.length > 0) {
                    for (File imgFile : imgFiles) {
                        if(imgFile.getAbsolutePath().contains(num)){
     ...
     * @return 是否注册成功
     */
    public boolean register(Context context, byte[] nv21, int width, int height, String name, String num, String uid) {
        synchronized (this) {
            Log.i("TJS", "======(faceEngine == null) = " + (faceEngine == null));
            Log.i("TJS", "======(context == null) = " + (context == null));
            Log.i("TJS", "======(nv21 == null) = " + (nv21 == null));
            Log.i("TJS", "======(width % 4 != 0) = " + (width % 4 != 0));
            Log.i("TJS", "======(nv21.length != width * height * 3 / 2) = " + (nv21.length != width * height * 3 / 2));
            ...
            Log.i("TJS", "======code = " + code);
            Log.i("TJS", "======(faceInfoList.size() > 0) = " + (faceInfoList.size() > 0));
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                try {
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        Log.i("TJS", "======(cropRect == null) = " + (cropRect == null));
                        if (cropRect == null) {
                            return false;
                        }
                        File file = new File(imgDir + File.separator + userName +"-"+num+"-"+uid+ IMG_SUFFIX);
                        ...
                        FileOutputStream fosFeature = new FileOutputStream(featureDir + File.separator + userName+"-"+num+"-"+uid);
                        ...
                        FaceRegisterInfo e = new FaceRegisterInfo(faceFeature.getFeatureData(), userName, num, uid, file.getAbsolutePath());

                        Log.e("Asion---info",e.toString());

                        faceRegisterInfoList.add(e);
                        return true;
                    }
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        if (maxSimilarIndex != -1) {
            ...
            Log.e("Asion---info","分数--"+maxSimilar+"--"+faceRegisterInfo.toString());
            return new CompareResult(faceRegisterInfo.getName(),faceRegisterInfo.getNum(),faceRegisterInfo.getPath(),faceRegisterInfo.getUid(),maxSimilar);
        }
        return null;
    }
