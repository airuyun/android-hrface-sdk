package com.dj.hrfacelib.util.camera;
...
public class CameraHelper implements Camera.PreviewCallback {
        ...
        synchronized (this) {
            ...
            try {
                ...
                if (supportedPreviewSizes != null && supportedPreviewSizes.size() > 0) {
                    previewSize = getBestSupportedSize(supportedPreviewSizes, previewView);
                }
    ...
    private Camera.Size getBestSupportedSize(List<Camera.Size> sizes, View previewView) {
        Camera.Size bestSize = sizes.get(0);
        float screenRatio = (float) previewView.getWidth() / (float) previewView.getHeight();
        if (screenRatio > 1) {
