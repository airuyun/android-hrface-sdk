...
public class DrawHelper {
    ...
    public DrawHelper(int previewWidth, int previewHeight, int canvasWidth,
                      ...
                      boolean isMirror,boolean isRight) {
    ...
    public Bitmap getRectBitmap(byte[] nv21, List<DrawInfo> drawInfoList, float rotation){
        if (nv21 == null || drawInfoList == null || drawInfoList.size() == 0) {
            return null;
        }
        Bitmap bitmap = ImageUtil.nv21ToBitmap(nv21, previewWidth, previewHeight);
        ...
        Bitmap rotatedRgbBitmap = Bitmap.createBitmap(bitmap , 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, false);

        ImageUtil.saveBitmap(Environment.getExternalStorageDirectory()+"/DJ_FACE/btm.jpg",rotatedRgbBitmap);

        return rotatedRgbBitmap;
    }
